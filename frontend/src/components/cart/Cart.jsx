...
import { addToCart, changeQuantity, removeFromCart } from "../../redux/reducers/user";
...
const Cart = ({ setOpenCart }) => {
  ...
  const removeFromCartHandler = async (data) => {
    ...
    await axios.get(`${server}/user/remove-item-cart/${data}`, { withCredentials: true });
    dispatch(loadUser());
  ...
  const quantityChangeHandler = async (id, quantity) => {
    ...
    await axios.get(`${server}/user/change-quantity-item-cart/${id}/${quantity}`, { withCredentials: true });
    const checkExisted = listSelected.find(item => item.product._id == id)

    if(checkExisted){
      const updatedItem = { ...checkExisted, quantity: quantity };
      const updatedList = listSelected.map((item) => (item.product._id === updatedItem.product._id ? updatedItem : item));
  ...
  const handleCheckboxChange = (itemCart) => {
    const checkExisted = listSelected.find(item => item.product._id == itemCart.product._id)
    if (checkExisted) {
      const newList = listSelected.filter((i) => i.product._id !== itemCart.product._id)
      setListSelected(newList)
    } else {
      setListSelected([
        ...listSelected, itemCart
      ]);
    }
  };
  ...
  useEffect(() => {
    const sumSelectedList = (() => {
      const total = listSelected.length > 0 ? listSelected.reduce(
  ...
  const checkOut = () => {
    if (listSelected.length === 0) toast.error("Please choose product to payment")
    else {
      localStorage.setItem("listSelected", JSON.stringify(listSelected));
      navigate("/checkout");
    }
        ...
        {cart && cart.length === 0 ? (
          ...
          <>
            <div>
              ...
              <div className="w-full border-t custom-dashboard">
                {cart && cart.map((i, index) => (
                  <CartSingle key={index} data={i} quantity={i.quantity} checkItemSelected={ checkItemSelected} quantityChangeHandler={quantityChangeHandler} removeFromCartHandler={removeFromCartHandler} handleCheckboxChange={handleCheckboxChange} />
                  ))}
              </div>
            </div>
            ...
            <div className="px-5 mb-3">
              {/* checkout buttons */}
              <div className={`h-[45px] flex items-center justify-center w-[100%] bg-[#e44343] rounded-[5px]`} onClick={checkOut}>
  ...
  return (
    <div className="border-b p-4">
      <div className="w-full flex items-center">
        ...
        {checkItemSelected(data.product._id) === false && (<RxCross1 className="ml-4 cursor-pointer" onClick={() => removeFromCartHandler(data.product._id)}/>)}
