...
const Checkout = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    setListSelected(localStorage.getItem("listSelected") ? JSON.parse(localStorage.getItem("listSelected")) : [])
  }, []);
  ...
  const subTotalPrice = listSelected.reduce(
  ...
  const handleSubmit = async (e) => {
    ...
    await axios.get(`${server}/coupon/get-coupon-value/${name}`).then((res) => {
      ...
      if (res.data.couponCode !== null) {
        const isCouponValid = cart && cart.filter((item) => item.product.shop === shopId);
    ...
    <div className="w-full flex flex-col items-center py-8">
      <div className="w-[90%] 1000px:w-[70%] block 800px:flex">
        <div className="w-full 800px:w-[65%]">
          <ShippingInfo user={user} country={country} setCountry={setCountry} city={city}
            setCity={setCity} userInfo={userInfo} setUserInfo={setUserInfo} address1={address1}
            setAddress1={setAddress1} address2={address2} setAddress2={setAddress2} zipCode={zipCode} setZipCode={setZipCode}
          />
        </div>
        <div className="w-full 800px:w-[35%] 800px:mt-0 mt-8">
          <CartData handleSubmit={handleSubmit} totalPrice={totalPrice} shipping={shipping} subTotalPrice={subTotalPrice}
...
const ShippingInfo = ({ user, country, setCountry, city, setCity, userInfo, setUserInfo, address1, setAddress1, address2,
  setAddress2, zipCode, setZipCode,}) => {
  return (
        ...
        <div>
          {user && user.addresses.map((item, index) => (
              <div className="w-full flex mt-1">
                <input type="checkbox" className="mr-3" value={item.addressType}
                  onClick={() =>
  ...
  return (
    <div className="w-full bg-[#fff] rounded-md p-5 pb-8">
      <div className="flex justify-between">
        ...
        <h5 className="text-[18px] font-[600]">{subTotalPrice.toLocaleString()} VND</h5>
      </div>
      <br />
      <div className="flex justify-between">
        <h3 className="text-[16px] font-[400] text-[#000000a4]">shipping:</h3>
        <h5 className="text-[18px] font-[600]">{shipping.toLocaleString()} VND</h5>
      </div>
      <br />
      ...
      <h5 className="text-[18px] font-[600] text-end pt-3">{totalPrice.toLocaleString()} VND</h5>
      <br />
      <form onSubmit={handleSubmit}>
        <input type="text" className={`${styles.input} h-[40px] pl-2`} placeholder="Coupoun code"
          value={couponCode} onChange={(e) => setCouponCode(e.target.value)}
        />
        <input className={`w-full h-[40px] border border-[#f63b60] text-center text-[#f63b60] rounded-[3px] mt-8 cursor-pointer`}
          required value="Apply code" type="submit"
        />
      </form>
    </div>
  );
