...
const Header = ({ activeHeading }) => {
  const { isAuthenticated, user, cart } = useSelector((state) => state.user);
    ...
    <>
                    ...
                    return (
                      <Link to={`/product/${i._id}`}>
                        <div className="w-full flex items-start-py-3">
                          <img className="w-[40px] h-[40px] mr-[10px]"
                            src={`${i.images[0]}`}
                          />
                          <h1>{i.name}</h1>
                        </div>
          ...
          <div className="flex">
            ...
            <div className={`${styles.noramlFlex}`}>
              <div className="relative cursor-pointer mr-[15px]">
                {isAuthenticated ? (
                  <Link to="/profile">
                    {user?.avatar ? (<img src={`${user?.avatar}`} className="w-[35px] h-[35px] rounded-full" />)
      ...
      <div className={`${ active === true ? "shadow-sm fixed top-0 left-0 z-10" : null } w-full h-[60px] bg-[#fff] z-50 top-0 left-0 shadow-sm 800px:hidden`}>
        <div className="w-full flex items-center justify-between">
          ...
          <div>
            <Link to="/">
              <img src="https://shopo.quomodothemes.website/assets/images/logo.svg" className="mt-3 cursor-pointer"/>
            </Link>
          </div>
          <div>
        ...
        {open && (
          <div className={`fixed w-full bg-[#0000005f] z-20 h-full top-0 left-0`} >
            <div className="fixed w-[70%] bg-[#fff] h-screen top-0 left-0 z-10 overflow-y-scroll custom-dashboard">
              <div className="my-8 w-[92%] m-auto h-[40px relative]">
                <input type="search" placeholder="Search Product..." value={searchTerm}
                  className="h-[40px] w-full px-2 border-[#3957db] border-[2px] rounded-md"
                  onChange={handleSearchChange}
                />
                {searchData && (
                      ...
                      return (
                        <Link to={`/product/${Product_name}`}>
                          <div className="flex items-center">
                            <img src={i.image_Url[0]?.url} className="w-[50px] mr-2" />
                            <h5>{i.name}</h5>
                          </div>
                        </Link>
                      );
              ...
              <div className={`${styles.button} ml-4 !rounded-[4px]`}>
                <Link to="/shop-create">
                  <h1 className="text-[#fff] flex items-center">
                    {isSeller ? "Go to Dashboard" : "Become Seller"} <IoIosArrowForward className="ml-1" />
                  </h1>
                </Link>
              </div>
              <br />
              ...
              <div className="flex w-full justify-center">
                {isAuthenticated ? (
                  <div>
                    <Link to="/profile">
                      <img src={`${user.avatar}`} className="w-[60px] h-[60px] rounded-full border-[3px] border-[#0eae88]"/>
                    </Link>
                  </div>
                ) : (
                  <>
